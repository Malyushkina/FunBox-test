...
4. Изображения контентные – в разметке, декоративные – в стилях. Размер изображений оптимизирован насколько это возможно с минимальной потерей качества (в зависимости от конкретной ситуации, мелкие превьюшки можно сжимать сильнее). Все img обёрнуты в div. Для оптимизации загрузки и ретинизации используется тэг picture. Там же предусматриваем возможность загрузки изображений в формате webp.
5. Все шрифты подключены локально. Для ускорения загрузки используется link rel=”preload”, и font-display: swap; чтобы можно было прочитать текст до того как наш шрифт полностью загрузится.
...
5. Оптимизировать изображения. Ретинизация. В контентных изображениях используем тэг picture, для ретинизации фоновых декоративных изображений используем медиазапрос @media (min-resolution: 2dppx)
